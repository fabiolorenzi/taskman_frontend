...
function CreateIteration() {
    ...
    const submitButton = e => {
        e.preventDefault();
        fetch(`https://taskman-backend.hopto.org/api/v1/iterations/${session.data.user}`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Accept": "application/json"
    ...
    useEffect(() => {
        if (result) {
            if (result.data) {
                alert(`The iteration ${result.data.version} has been created successfully.`);
